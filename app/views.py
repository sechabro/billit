...
class Queries:
    ...
    '''# LOGIN CHECK ------------------------------------
    @app.route('/login/<str:user>', methods=['GET', 'POST'])
    def login(user: str):
        username = UserModel.query.filter(UserModel.name == user)
    ...
    def invoice_index():
        ...
        title='All Invoices'
        return render_template('admin/templates/dashboard_template.html', title=title, invoice_list=invoice_list, client_list=client_list, paid_inv=q.paid_inv, unpaid_inv=q.unpaid_inv, total_inv=q.total_inv,
                               total_clients=q.total_clients, billed_total=q.billed_total, collected_total=q.collected_total)

    @app.route('/dashboard/unpaid', methods=['GET', 'POST'])
    def unpaid_view():
        q.gen_query()
        invoices = InvoiceModel.query.filter(InvoiceModel.paid == False)
        client_select = ClientModel.query.all()
        invoice_list = q.list(invoices)
        client_list = q.list(client_select)
    ...
    def client_view():
        q.gen_query()
        invoices = InvoiceModel.query.order_by(InvoiceModel.client)
        client_select = ClientModel.query.all()
        invoice_list = q.list(invoices)
        client_list = q.list(client_select)
